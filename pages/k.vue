...
import { resolve } from '~/static/polyfills.js';
   ...
   async getPicture(){
    ...
    async fetchData() {
      ...
      return new Promise((resolve, reject) => {
        function roundToTimeFrame(timestamp, timeFrameInMinutes) {
  let date = new Date(timestamp);
  let minutes = date.getMinutes();
  let roundedMinutes = minutes - (minutes % timeFrameInMinutes);
...
function calculateStartTime(endTime, timeFrameInMinutes, count) {
  let totalMilliseconds = timeFrameInMinutes * 60 * 1000 * count;
  return new Date(endTime.getTime() - totalMilliseconds);
}

// Hàm để lấy ra các kết quả tuần tự và lưu vào mảng
function getTimePeriods(initialTimestamp, timeFrame, count, loops) {
  let timePeriods = [];
  let currentTimeStamp = initialTimestamp;

  for (let i = 0; i < loops; i++) {
    let endTime = roundToTimeFrame(currentTimeStamp, timeFrame);
    let startTime = calculateStartTime(endTime, timeFrame, count);

    timePeriods.push({ startTime: startTime, endTime: endTime });

    // Cập nhật currentTimeStamp cho vòng lặp tiếp theo
    currentTimeStamp = startTime.getTime() - 10;
  }
...
// Sử dụng hà<main>
function createTimeQuery(timePeriods, symbol) {
  return timePeriods.map(period => {
    let startTime = period.startTime.getTime();
    let endTime = period.endTime.getTime();
    return `https://fapi.binance.com/fapi/v1/klines?&interval=5m&limit=1000&symbol=${encodeURIComponent(symbol)}&startTime=${startTime}&endTime=${endTime}`;
  });
}
let timeFrame = 5;
...
// Sử dụng hàm với các giá trị từ trước
let initialTimeStamp = new Date().getTime(); // Sử dụng timestamp hiện tại
 // Khung thời gian 5 phút
let count = 1000; // Số lượng kết quả mỗi chu kỳ
let loops = this.count; // Số lần lặp

let periods = getTimePeriods(initialTimeStamp, timeFrame, count, loops);
let symbol = this.symbol; // Tham số symbol cho truy vấn
let queries = createTimeQuery(periods, symbol);

let fetchPromises = queries.map(query => {
  return new Promise((resolve, reject) => {
...
Promise.all(fetchPromises).then(data=>{
  ...
  let rawData =data.flat()

 rawData= rawData.sort((a, b) => a[0] - b[0]);
let final=(this.formatData(rawData))
...
//console.log(fetchPromises);
    ...
    initChart() {
      if (this.$store.state.db.list_symbol_config[this.symbol] === undefined) {
        ...
        this.fetchData().then((data) => {
          ...
          chart.setPriceVolumePrecision(optionFromExchange.pricePrecision,optionFromExchange.pricePrecision/10 );
          chart = chart;
  ...
  mounted() {
    ...
    let count=this.$route.query.count||10;
    this.symbol = symbol;
